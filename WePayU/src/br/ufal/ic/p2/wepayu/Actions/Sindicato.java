...
public class Sindicato {
    ...
    public static void remover(String id){
        membros.removeIf(s -> s.getId().equals(id));
    }

    /**
     * Metodo responsavel por colocar um novo empregado no sindicato
     */
    public static void alterarSindicato(String emp, String atributo, String valor, String idSindicato, String taxaSindical) throws IdentificacaoDoEmpregadoNaoPodeSerNulaException {
        ...
        if (idSindicato.isEmpty()) throw new IdentificacaoDoSindicatoNaoPodeSerNulaException();
        if (taxaSindical.isEmpty()) throw new TaxaSindicatoNaoPodeSerNulaException();

        try{
            double aux = Double.parseDouble(taxaSindical.replace(",", "."));
            if (aux <= 0) throw new TaxaSindicalDeveSerNaoNegativaException();
